'use client';

import { HomeIcon, PuzzlePieceIcon, CurrencyRupeeIcon, WalletIcon } from '@heroicons/react/24/outline';
import Link from 'next/link';
import { getBasePath } from '@/app/lib/utils';

const links = [
    { name: 'Home', href: '/', icon: HomeIcon },
    { name: 'Matches', href: '/matches', icon: PuzzlePieceIcon },
    { name: 'Earnings', href: '/earnings', icon: CurrencyRupeeIcon },
    { name: 'Wallet', href: '/wallet', icon: WalletIcon },
];

export default function NavLinks() {
    const basePath = getBasePath();

    return (
        <>
            {links.map((link) => {
                const LinkIcon = link.icon;
                return (
                    <Link
                        key={link.name}
                        href={link.href}
                        className={`flex h-[48px] grow items-center justify-center gap-2 rounded-md bg-gray-50 p-3 text-sm font-medium hover:bg-sky-100 hover:text-blue-600 md:flex-none md:justify-start md:p-2 md:px-3 ${basePath === link.href ? 'bg-sky-100 text-blue-600' : ''}`}
                    >
                        <LinkIcon className="w-6" />
                        <p className="hidden md:block">{link.name}</p>
                    </Link>
                );
            })}
        </>
    );
}